...
int main(int argc, char **argv)
{
    ...
    fdnull = open("/dev/urandom", O_RDONLY);
    if (fdnull < 0)
        return pr_perror("Can't open /dev/urandom");
    ...
    if (ptrace(PTRACE_GETREGSET, pid, NT_PRSTATUS, &iov))
        return pr_perror("Can't get child registers");
    if (regs.a0 != fdzero)
        return pr_fail("unexpected a0: %lx, fdzero: %x, fdnull: %x", regs.a0, fdzero, fdnull);
